...
class SimpleHTTest234 {
    ...
    public static void main(String[] args) {
        ...
        Generator<String> stringGen = () -> rng.ints(48, 123).filter(i -> (i <= 57 || i >= 65) && (i <= 90 || i >= 97))
                .limit(42)
        ...
        Integer a = rand.nextInt(p - 1) + 1;
        Integer b = rand.nextInt(p);

        testHT(intGen, intGen, new SimpleHT<Integer, Integer>(capacity,
                (key) -> {
                    // h(x) = (a * key + b) mod p = a * key mod p + b mod p
                    long c = (long) key * a;
                    c = c % capacity;
                    int bb = b % capacity;
                    return (int) (c + bb);
        ...
        testHT(stringGen, floatGen, new SimpleHT<String, Float>(capacity,
                (key) -> {
                    int c = key.charAt(key.length() - 1) * a;
